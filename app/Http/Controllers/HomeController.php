    ...
    public function index()
    {
        ...
        if(auth()->user()->user_type == 'admin')
		{
		  ...
		  //  $reviews                    =   Review::inRandomOrder()->take(5)->get();
		    $reviews                    =   Review::distinct()->inRandomOrder()->get();


		    
		    
		  //  echo "<pre>";
		  //  print_r($reviews->toArray());
		  //  echo exit;
            $orderCount                 =   Orders::where('cancel_status', 'no')->count();
            ...
            $successorderCount          =   Orders::where('is_paid','yes')->where('cancel_status','no')->count();
            ...
            $orderearningsFormatted     =  $orderearnings;
...
*/
    ...
    public function getMonthlyStats(Request $request)
    {
        ...
        $query = Orders::select('created_at', 'total_amount', 'cancel_status','is_paid');
        ...
        $monthlyStats = $monthsKeys->map(function ($monthKey) use ($grouped) {
            ...
            return [
                'month' => Carbon::parse($monthKey . '-01')->format('M Y'),
                'totalamountcomes' => $ordersInMonth->sum('total_amount'),
                'success_orders' => $ordersInMonth->where('is_paid', 'yes')->where('cancel_status', 'no')->sum('total_amount'),
